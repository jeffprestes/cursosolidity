...
contract Mortal is Owned  {
    ...
    function setStopStart() public {
        require(msg.sender==contractOwner, "Only owner can stop/start the contract");
        if (isPaused) {
            isPaused = false;
        } else {
            isPaused = true;
    ...
    modifier contractIsNotPaused {
        require(!isPaused, "Contract operation is paused");
        _;
    }
}
...
contract USDC is IERC20Metadata, Mortal {
    ...
    function transfer(address to, uint256 amount) public override contractIsNotPaused hasEnoughBalance(msg.sender, amount) tokenAmountValid(amount) returns(bool) {
    ...
    function approve(address spender, uint limit) public override contractIsNotPaused returns(bool) {
    ...
    contractIsNotPaused hasEnoughBalance(from, amount) isAllowed(msg.sender, from, amount) tokenAmountValid(amount)
    returns(bool) {
    ...
    function mint(address account, uint256 amount) public onlyOwner contractIsNotPaused returns (bool) {
    ...
    function burn(address account, uint256 amount) public onlyOwner contractIsNotPaused returns (bool) {
